...
def process_category(category: str, feed_configs: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    # Process articles with rate limiting for free tier
    max_workers = PIPELINE_CONFIG.get("max_concurrent_workers", 1)
    rate_limit_delay = PIPELINE_CONFIG.get("rate_limit_delay", 4.5)

    if max_workers == 1:
        # Sequential processing with rate limiting
        logger.info(f"Processing {len(all_articles)} articles sequentially with {rate_limit_delay}s delay")
        for i, article in enumerate(tqdm(all_articles, desc=f"Processing {category} articles")):
            try:
                market = process_article(article)
                if market:
